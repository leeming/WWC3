...
(function($){
  ...
  jQuery.fn.containerSetButtons = function (buttons,opt){
    ...
    if (buttons !=""){
      ...
      for (var i in btn){
        ...
        if (btn[i]=="r"){
          container.find(".buttonBar:first").append("<img src='"+path+container.attr('skin')+"/reload.gif' class='reloadContainer'/>");
  ...
  jQuery.fn.containerReload = function (opt){
    
      var container=$(this);
      container.mb_changeContainerContent("lobby.php", history[curIndex]);
  };
  
  jQuery.fn.setHistory = function (page){
    //check to see if at last page of history
    
    //clear all 'forward' history
    if(history[curIndex+1] != undefined)
    {
      for(var i = curIndex; i < history.length; i++ )
      {
        history[i] = undefined;
      }
    }
    ...
    curIndex++;
    history[curIndex] = new Array();
    history[curIndex] = page;
